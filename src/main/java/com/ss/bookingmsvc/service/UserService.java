package com.ss.bookingmsvc.service;

import com.ss.bookingmsvc.dao.UserDAO;
import com.ss.bookingmsvc.entity.User;
    ...
    public Boolean addUser(User u) throws ClassNotFoundException, SQLException {
        ...
        try {
            ...
            Integer id = uDAO.saveWithPK("INSERT INTO user SET  role_id = (SELECT id FROM user_role WHERE id = ?)," +
                            "given_name = ?, family_name = ?, username = ?, email = ?, password = ?, phone = ?, roles =?",
                    (new Object[] {u.getRole_ID(), u.getGiven_Name(), u.getFamily_Name(), u.getUsername(), u.getEmail(),
                            u.getPassword(), u.getPhone(), u.getRoles()}));
            conn.commit();
            return true;
        } catch (Exception e) {
            e.printStackTrace();
